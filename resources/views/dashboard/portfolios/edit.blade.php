        ...
        <div class="form-card">
            ...
            <form method="POST" action="{{ route('portfolio.update', $portfolio) }}" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    ...
                    <input type="date" id="created_at" name="created_at" class="form-input" value="{{ old('created_at', $portfolio->created_at ? $portfolio->created_at->format('Y-m-d') : now()->format('Y-m-d')) }}">
                    @error('created_at') <span class="error-msg">{{ $message }}</span> @enderror
                </div>
